...
delays = []
for i in range(3):
    delays.append((timeAveragedcrossCorrelation(micSignals[i], micSignals[i + 1], returnDelay=True, graphs=True)[1]) / fs)
...
delays = []
for i in range(3):
    delays.append((generalizedCrossCorrelation(micSignals[i], micSignals[i + 1], mode='scot', returnDelay=True, graphs=True)[1]) / fs)
...
delays = []
for i in range(3):
    delays.append((generalizedCrossCorrelation(micSignals[i], micSignals[i + 1], mode='phase', returnDelay=True, graphs=True)[1]) / fs)
